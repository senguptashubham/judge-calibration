...
**Final correction, 23 Sep 2026 — turn=2 is dropped from this arm entirely, superseding this section's own "both turn populations are always reported" sentence above.** A 100-item smoke test (run after the fix above) found a genuine output-length parse-failure problem, distinct from the input-length issue just described: 0% failure in `clean` (both turns), 7.1% in verbose/turn=1, **36.1% in verbose/turn=2** — every failure traces to `finish_reason=="length"` (truncated at `max_tokens=1024` before reaching a decision line), and spot-checked examples showed genuine model-level degenerate token repetition in several cases, not just "ran out of room near a natural end." Since `clean`/turn=2 was itself perfectly reliable (0%, identical to `clean`/turn=1), this is a `verbose`×turn=2 interaction, not a turn=2 defect — and the real, considered trade-off (keep `clean`/turn=2's good data and the interesting turn=1-vs-turn=2 verbosity-degradation differential, vs. simplify the arm and cut the remaining runtime by roughly half) was decided in favor of simplicity, given the remaining project timeline. **Population is now 1,904 → ~948 items, turn=1 only, both orders, both conditions.** No coverage-regime split (unchanged from above — auto-j's context comfortably covers the turn=1 population). Full trade-off analysis and the decision record in `DECISIONS.md` D28's third 23 Sep amendment.
