...
1,904 items, both orders, both conditions (`clean`+`verbose`, D28). Results are stratified by **`turn`** (1 vs. 2): auto-j's own template takes one flat `{prompt}` field with no native multi-turn support, so MT-Bench's turn=2 items require an explicit, novel flattening design (D28) that has no precedent in auto-j's own materials. **Both turn populations are always reported** — never silently merged or dropped — and turn=2's validity is judged by a pre-committed, outcome-independent check (parse-failure rate + manual spot-check, task L4b) decided *before* any calibration number exists for either turn, specifically to avoid deciding "keep vs. discard" based on how the numbers look (D28).

**Correction, 23 Sep 2026 (D28's own amendment) — auto-j's context does NOT cover 100% of both conditions; this section originally, wrongly, said it did.** The claim was never actually measured before being written here. Real, offline-measured impact (full population, auto-j's own tokenizer): **181/3,808 rows (4.75%) exceed the 7,168-token generation-budget-reserving ceiling**, almost entirely concentrated in `verbose`/turn=2 (164/956, ~17.2% of that subgroup) — the compounding effect of this section's own turn=2 flattening design (each side's response field folds both turns' content) combined with `verbose_pad()` padding both of those folded turns independently. `clean` and `verbose`/turn=1 stay comfortably within bounds. Excluded calls are skipped and logged explicitly (`src/judge_autoj.py::split_by_prompt_length()`, mirrors `judge_kev.py`'s own `max_state_tokens` precedent), never silently dropped — and this exclusion rate is reported as a real, stated limitation in `REPORT.md`'s RQ7 section (L6), not minimized. Full trail in `DECISIONS.md` D28.

**Correction (interim, later reversed), 23 Sep 2026 — turn=2 was dropped from this arm entirely for a few hours, superseding this section's own "both turn populations are always reported" sentence above.** A 100-item smoke test (run after the fix above) found a genuine output-length parse-failure problem, distinct from the input-length issue just described: 0% failure in `clean` (both turns), 7.1% in verbose/turn=1, **36.1% in verbose/turn=2** — every failure traces to `finish_reason=="length"` (truncated at `max_tokens=1024` before reaching a decision line), and spot-checked examples showed genuine model-level degenerate token repetition in several cases, not just "ran out of room near a natural end." Since `clean`/turn=2 was itself perfectly reliable (0%, identical to `clean`/turn=1), this is a `verbose`×turn=2 interaction, not a turn=2 defect.

**Reversed the same day, once the real cost-benefit changed.** The full-run command was accidentally started on the OLD (both-turns) code before the turn=1-only fix had been pulled into that already-running process — `git pull` doesn't affect an already-running Python process — and it ran that way for 7 hours before the mismatch was caught (no data-integrity problem: zero duplicate checkpoint keys). At that point ~54% of the full 1,904-item, both-turns population was already done, and the remaining-time gap between "finish both turns" (~5.9h more) and "restart turn=1-only" (~2.9h more) had shrunk to ~2.5-3 hours — small enough that finishing both turns (recovering `clean`/turn=2's good data and the turn=1-vs-turn=2 differential) was judged worth the small extra time. **Population is 1,904 items, both turns, both orders, both conditions — unchanged from this section's original statement.** No coverage-regime split (auto-j's context comfortably covers the population). The 36.1% verbose/turn=2 finding itself stands regardless of this reversal and must still be reported. Full back-and-forth in `DECISIONS.md` D28's third and fourth 23 Sep amendments.
