...
**Measured budget.** Task 1.6's pilot on a Colab L4 gave ~1.2 sec/generation once the ~36s model-load cost is separated out; `verbose` measured slightly faster, but clean's rate was kept as the planning number. At the real N = 1,904: 1,904 × 12 × 3 (rerun factor) = 68,544 generations ≈ 22.9 L4-hours ≈ 35 compute units, against 72.8 available — N did not need cutting (D12). The run spans several Colab sessions, which is what checkpoint/resume (invariant 9) is for.
...
⚠️ **Feature-parity fix required (D21):** `clean` has `conf_sc` and `conf_ens` (+ its decomposition); `verbose` has neither, since self-consistency and the P2/P3 ensemble are both clean/P1-only under D19's schedule. This transfer test uses **Tier A minus `{conf_sc, conf_ens, ens_entropy_total, ens_entropy_aleatoric, ens_entropy_epistemic}`** for every model compared — not a Bayesian-specific carve-out. State this explicitly in `REPORT.md`'s limitations; never silently impute or leave NaN.
...
- **Permutation null:** shuffle `correct` within each `question_id` (so each question keeps its error rate), re-run the grouped CV, 200 permutations; report the null AUROC distribution and where your number sits in it.
...
Fit with NumPyro/NUTS, run under the **same** `StratifiedGroupKFold(5)`×10-seed protocol as §2.5 — the hierarchical structure adds partial pooling on top of that CV protocol, it does not replace it. Features are standardized on each training fold, exactly as for LogReg — `β ~ Normal(0, 1)` only means the same thing for every feature once they share a scale.
...
*Amended 25 Sep 2026 (D29):* the demo became a static HTML/CSS/JS site (`site/`) instead of a Gradio app, and the live model call was skipped. It is published on GitHub Pages; the repo is public under MIT (code) and CC BY 4.0 (text and figures).
...
1. Second judge model — cut from the core plan; later added as owner-initiated RQ6/RQ7 (§7–8)
...
1,904 items, both orders, both conditions (`clean`+`verbose`, D28), **both turns, always reported separately**. auto-j's template takes one flat `{prompt}` field with no native multi-turn support, so turn=2 items use a flattening design of this project's own (D28): the shared prompt field carries both user turns, and each side's response field carries that side's two answers.

Two exclusions, both logged rather than silently dropped, and both reported in `REPORT.md`:
- **Input length:** 4.75% of calls exceed the 7,168-token prompt ceiling (8,192 context minus the 1,024-token generation budget) and are skipped by `src/judge_autoj.py::split_by_prompt_length()`. Almost all are `verbose`/turn=2, where padding applies to both folded turns.
- **Output length:** auto-j produces no parseable verdict on 39.8% of `verbose`/turn=2 calls (13.6% `verbose`/turn=1, ~0% `clean`) — every failure is a completion truncated at `max_tokens` before its decision line.

Turn=2 was briefly dropped mid-run after a smoke test found the second problem, then restored once most of the both-turns run had already completed; the full back-and-forth is in `DECISIONS.md` D28.
