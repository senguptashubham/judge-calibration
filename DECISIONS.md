...
## D29 ⚑ — The demo is a static site, not a Gradio app; a post-hoc auto-accept analysis; the repo goes public under MIT + CC BY *(owner decisions, 24–25 Sep 2026)*

**Auto-accept analysis (owner-initiated).** The owner asked for the deployment-side reading of RQ1–RQ7: if a pipeline auto-accepts every verdict above a confidence threshold, what share of the wrong verdicts slip through? Added as `analysis/auto_accept.py` over `src/metrics.py::auto_accept_stats`, with cluster-bootstrap CIs over `question_id`. **It is a post-hoc, descriptive analysis and is labelled so in `REPORT.md`** — not preregistered, no new hypothesis test, no signal-vs-signal ordering claimed. Calibration forms only (D7 amendment): a threshold on 1 − H is meaningless. The meta-models enter through their out-of-fold P(correct), which `analysis/rq4.py` now also saves (the CV and seeds are unchanged, so no RQ4 number moved). At ≥ 0.90, Qwen's stated confidence accepts 99% of verdicts and lets 98.4% [97.3, 99.3] of the wrong ones through; this became the README's lead finding.

**Gradio dropped for a plain HTML/CSS/JS site.** A Gradio app was built first (task 6.2's original plan) and then replaced, on the owner's call, after reviewing it. The reasons: its layout could not be made to look intentional, it needed a Python server to show a fixed set of recorded results, and a static page loads fast on any device, costs nothing to host and opens straight from disk. The design rule carried over from the Gradio plan: **the site computes no statistic.** `analysis/demo_bundle.py` → `results/site_bundle.json.gz` (an intermediate, not in git) → `analysis/site_data.py` → `site/data/site-data.js` and `site/data/examples.js`, with every number from the project's own functions and padded answers rendered by `src/perturb.py::verbose_pad`. The random "Trick the judge" examples are fetched only on demand.

**Selections that could bias what a visitor sees are made blind to the judges' verdicts and with a fixed seed.** The game pool is one item per question from short turn-1 items where every human voter agreed; the random examples are a seeded draw under a size cap. The size cap favours short answers, so neither set supports a rate claim, and the site says so. The only rates it states are the measured ones.

**The optional live model call (task 6.3) was skipped**, on the owner's call: nothing on the site is generated live.

**Colour and accessibility.** The site's judge palette (`#2a78d6` / `#eda100` / `#1baf7a`) and red accent (`#d03b3b`) were validated together for colour-vision separation. matplotlib's `tab:orange`/`tab:green` were nearly identical for red-green colour-blind readers, so `src/plots.py` now uses the same palette; the judge-comparison and auto-accept figures were regenerated, with byte-identical result tables. Dark mode re-steps only what fails on a dark surface: kev amber → `#c98500`, and red *text* → `#ea6a64`. Where text sits on a judge colour, a deeper fill shade of the same hue is used, chosen by APCA: black on the teal and amber scored Lc ~50, too faint at 12px.

**The repo goes public** (owner confirmed permission, 25 Sep 2026): code under MIT; text and figures under CC BY 4.0, matching the CC BY 4.0 MT-Bench data. The site is published from `site/` by `.github/workflows/pages.yml`, and as a static Hugging Face Space by `scripts/deploy_space.py` (https://huggingface.co/spaces/shubhamsengupta/judge-calibration; first deployed by the owner, 25 Sep 2026).
